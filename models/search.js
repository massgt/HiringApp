...
/**`SELECT dbengineer.id_engineer, dbengineer.name_engineer, dbengineer.desc_engineer,
            dbengineer.loc_engineer, GROUP_CONCAT(skill.Skill) as Skills, dbengineer.dateofbirth, dbengineer.date_created,
            dbengineer.date_update FROM \`dbengineer\` LEFT JOIN \`skill\` ON dbengineer.id_engineer = skill.id_engineer
            WHERE dbengineer.name_engineer LIKE '%A%' AND skill.Skill LIKE '%C%' GROUP BY
            dbengineer.id_engineer ORDER BY \`dbengineer\`.\`name_engineer\` ASC, \`Skills\` ASC` */